<?php
class Lqc_plan_model extends CI_Model {
    ...
    function update_plan($data, $plan_id){
        $needed_array = array('product_id', 'supplier_id', 'part_id', 'plan_date','lot_size','part_no');
        $data = array_intersect_key($data, array_flip($needed_array));
    ...
    function get_all_plans_lqc($product_id, $supplier_id, $plan_date = '',$part_id = '') {
        $sql = "SELECT pl.*, 
        p.code as part_no, p.name as part_name
	...
	function get_all_plans_lqc_new($product_id, $supplier_id,$part_id = '') {
        $sql = "SELECT pl.*, 
        p.code as part_no, p.name as part_name
        FROM production_plans pl
        INNER JOIN sqim_new.product_parts p
    ...
    function get_plan_lqc($plan_id, $supplier_id = '') {
        $sql = "SELECT pl.*, 
        p.code as part_no, p.name as part_name, s.name as supplier_name
        FROM production_plans pl
        INNER JOIN sqim_new.product_parts p
    ...
    function delete_plan_lqc($plan_id) {
        if(!empty($plan_id)) {
            $this->db->where('id', $plan_id);
    ...
    function get_plan_lqc_by_part($part_id,$plan_date,$supplier_id = '') {
        if(!empty($part_id)) {
			
			 $sql = "SELECT *  FROM production_plans
               WHERE part_id = ? AND plan_date = ? ";
					$pass_array[] = $part_id;
					$pass_array[] = $plan_date;
				if(!empty($supplier_id)) {
					$sql .="  AND supplier_id = ?";
	...
	function delete_plan_lqc_by_part($part_id,$plan_date,$supplier_id = '') {
        if(!empty($part_id)) {
            $this->db->where('part_id', $part_id);
    ...
    function change_lqc_plan_status($data, $status) {
        if(!empty($data) && !empty($status)) {
            $this->db->where('product_id',$data['product_id']);
            $this->db->where('supplier_id',$data['supplier_id']);
            $this->db->where('part_id',$data['part_id']);
            $this->db->set('plan_status', $status);
			// UPDATE `production_plans` SET `plan_status`='srated' WHERE product_id = '1' AND supplier_id = '19' AND part_id = '544'
					
            return $this->db->update('production_plans');
        }
    ...
    function get_timecheck_counts($product_id,$plan_date){
	 ...
	 function insert_production_plan_lqc($production_plans, $production_plan_date) {
