                       	...
                       	<div class="row" >
							...
							<?php  //	print_r($lqc_defect_code); ?>
										...
										?>
											...
											<div class="col-sm-4" >
													...
													<?php if(array_search($button_txt,$df)){
														...
														echo 'unpunched';
													} ?>"	data-toggle="button" defect_code_id ="<?php echo $dc['id']; ?>" autocomplete="off" 
													aria-pressed="<?php if(array_search($button_txt,$df)){
														echo 'true';
													}else{
														echo 'false';
													} ?>"	onclick="return change_bg(this);"	 >
															  <?php echo $val_b; ?>
								...
								<div class="form-group">
                                        ...
                                        <textarea id="register-inspection-remark"  onblur="return check_space(this);" name="remark" placeholder="Remarks" rows="2" cols="70"><?php echo $lqc_defect_code['remark'];	?>	</textarea>
								...
								<div class="form-group">
									...
									<a style="margin-top:20px;" class="btn btn-circle default" href="<?php echo base_url();	?>auditer/finish_screen_lqc" >Cancel</a>
								</div> 
							</div>
						</div>
					</div>
...
function submit_inspection(i,audit_id,al_id)
{
	...
	if(defect_occured.length > 0){
		...
		$.ajax({
			type: 'POST',
			url: 'update_submit_defect',
			data: { result : result , defect_occured : defect_occured,defect_occured_ids:defect_occured_ids,remark:remark,al_id:al_id },
			dataType: 'json',
				...
				if(resp['audit_def_id'] > 0){
					...
					bootbox.alert({ 
					  ...
					  callback: function(){
						  ...
						  ?>
						window.location.href = "<?php echo base_url();	?>auditer/finish_screen_lqc";					  
					  }
					});
