<?php 
	if(count($checkpoints_res_ok) == count($checkpoints)) {
		
		$ul = base_url().'auditer/mark_as_complete/'.$audit_id;
	}
	else{
		$ul = base_url().'auditer/mark_as_complete';
	}
?>
            ...
            <div class="portlet light bordered">
                ...
                <div class="portlet-title">
                    ...
                    <?php if(!isset($admin_edit_audit)) { ?>
                        <div class="actions">
							<input type="checkbox" name="send_gerp" id="send_gerp" onclick="on_handle_checkbox(this,<?php echo $audit_id; ?>);" 
							<?php 
								if(count($checkpoints_res_ok) == count($checkpoints)) {	?>  checked='checked' value='1' <?php }
								else{ ?> disabled='disabled' value='0' <?php	}
							?>
							>
							<span style="font-size:14px" >Send to GERP</span>
							
                            <a id="mark_comp" href="<?php echo $ul; ?>" data-confirm="Are you sure you want to mark this inspection result as complete. Once marked as complete the inspection result can't be changed." class="button normals btn-circle">    
                                Mark As Completed
                            </a>
                ...
                <div class="portlet-body form">
                    <table class="table table-hover table-light">
                        ...
                        <tbody>
                            <tr class="warning">
                                <td colspan="12">LG Checkpoints</td>
                            </tr>
                            <?php $first = true; ?>
                            <?php 
							...
							foreach($checkpoints as $checkpoint) { ?>
                                ...
                                <?php if($checkpoint['checkpoint_type'] == 'Supplier' && $first) { ?>
                                    <tr class="warning">
                                        <td colspan="12">Suppliers Checkpoints</td>
                                    </tr>
                                    <?php $first = false; ?>
                                <?php } ?>
                                <?php 
                    ...
                    <?php if(!isset($admin_edit_audit)) { ?>
                        <div class="form-actions right">
                            <a  id="mark_comp1"  href="<?php echo $ul; ?>" data-confirm="Are you sure you want to mark this inspection as complete. Once marked as complete the inspection can't be changed." class="button normals btn-circle">    
                                Mark As Completed
                            </a>
                        </div>
                    <?php } ?>
