<script>
	...
	function check_dups(){
		//alert("123");
		var check_inspect_id=$("#inspection_spec_detail_id").val();
		//alert(check_inspect_id);
		//var part_id = $('#part_id :selected').val();
        $.ajax({
			type:'post',
				url:'check_inspection_detail_id',
				data:{check_inspect_id: check_inspect_id},
				success:function(resp){
						//alert(resp); 
						if(resp == '{"exist":1}'){
											
							bootbox.dialog({
											message: 'Entered Inspection Spec Detail ID already exist. Please enter unique Inspection Spec Detail ID.',
											title: 'Alert',
											buttons: {
												confirm: {
													label: "OK",
                    ...
                    <form role="form" id="add-checkpoint-form" class="validate-form" method="post" enctype="multipart/form-data">
                        <div class="form-body">
                            ...
                            <div class="row">
								...
								<div class="col-md-6">
                                    <div class="form-group">
                                        <label class="control-label" for="inspection_spec_detail_id">Inspection Spec Detail ID :
											<span class="required">*</span>
										</label>
                                        <input type="text" class="required form-control" onkeydown="return isNumberKey(event);"  onblur="return check_dups();" id="inspection_spec_detail_id" name="inspection_spec_detail_id" title="Only Numbers allowed here."
                                        value="<?php echo isset($checkpoint['inspection_spec_detail_id']) ? $checkpoint['inspection_spec_detail_id'] : ''; ?>">
                                        <span class="help-block">
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group" id="inspection-config-sel-type-error">
                                        ...
                                        <select id="inspection-config-sampling-type" name="sampling_type" class="form-control required select2me"
											...
											<option value="TQI" <?php if($sampling_type == 'TQI') { ?> selected="selected" <?php } ?>>TQI</option>
                                        </select>
                                    </div>
                                </div>  
							...
							<!--div class="row type-specific-div" id="type-tqi-div" <?php if($sampling_type != 'TQI') { ?> style="display:none;" <?php } ?> >
                                
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label class="control-label" for="tqi_qty">Total Qty:
                                            <span class="required"> * </span>
                                        </label>
                                        <input type="text" class="required form-control" name="tqi_qty"
                                        value="<?php echo isset($inspection_config['sample_qty']) ? $inspection_config['sample_qty'] : ''; ?>">
                                    </div>
                                </div>
                        ...
                        <?php   if(!empty($checkpoint['part_id'])){
