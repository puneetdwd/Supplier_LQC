...
class Users extends Admin_Controller {
    ...
    public function __construct() {
        ...
        $this->template->write('title', 'SQIM | '.$this->user_type.' Users');
        ...
        if($this->input->post()) {
            ...
            if($validate->run() === TRUE) {
                ...
                if($response['status'] === 'SUCCESS') {
					$this->User_model->update_user_last_login($this->input->post('username'));
        ...
        if($this->input->post()) {
            ...
            if($validate->run() === TRUE) {
                ...
                if($response['status'] === 'SUCCESS') {
                    ...
                    // echo $this->input->post('sup_user_type');exit;
                    if($this->input->post('sup_user_type') == 'Supplier'){
						
						$this->User_model->update_supplier_last_login($this->input->post('username'));
						// echo $this->db->last_query();exit;
						redirect(base_url()); // logged in redirect to index page.
                    }else{
						$this->User_model->update_supplier_inspector_last_login($this->input->post('username'));
                        redirect(base_url()); // logged in redirect to index page.
                    }
                } else {
                   $data['error'] = $response['message'];
