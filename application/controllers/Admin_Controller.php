...
class Admin_Controller extends CI_Controller {
    ...
    function create_sample_qty($checkpoint_id, $part_id, $prod_lot_qty) {
        ...
        if(!empty($config)) {
			...
			if($config['sampling_type'] == 'TQI') {
                // $sample = $config['tqi_qty'];
                $sample = $prod_lot_qty;
            }
	...
	function hits($page_new){
		//For page hits
		$this->load->model('User_model');
		$page_hit = $this->User_model->get_page_count($page_new);
		
		$d['page_name'] = $page_new;
		if(!empty($page_hit)){
			//update
			$page_hit_in = $this->User_model->update_page_count($d,$page_hit['id'],$page_hit['hit_count']);
			
		}
		else{
			//insert
			$page_hit_in = $this->User_model->update_page_count($d);
