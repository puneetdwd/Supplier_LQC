...
function printPage_new(id,qr,part_id,supplier_id) {
	...
	$.ajax({
            type: 'POST',
            url: 'qr_code_generate/save_print_qr_history',
            data: { qr : qr,part_id:part_id,supplier_id:supplier_id},
            dataType: 'json',
            success: function(resp) {
				l = resp.length;
                // alert(l);
				
				res_str = resp.toString();
				console.log(res_str);
				// alert(resp);
				// document.getElementById("print_idds").innerHTML = res_str;
				$("#print_idds").attr("value", res_str);
 ...
 function map_part_foolproof_1(part_id,i){
	...
	function on_handle_checkbox(e,audit_id)
	{
		...
		if(s != 'checked'){
			...
			//var m = base_url+'auditer/mark_as_complete';//$('#mark_comp').attr('href');
			
			mm = m+'/'+audit_id;
			$('#mark_comp').prop("href", mm);
			$('#mark_comp1').prop("href", mm);
			// var m = $('#mark_comp1').attr('href');
		}
		else{
			//alert(456);
	...
	function printPage_by_view(data) {
		alert(data);
    var html="<html>";
    html+= data;
    html+="</html>";
